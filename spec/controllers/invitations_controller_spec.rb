  ...
  describe "POST create" do
    ...
    describe "with valid params" do
      it "assigns a newly created invitation as @invitation" do
        Invitation.stub(:new).with({'these' => 'params'}) { mock_invitation(:save => true) }
  ...
  describe "PUT update" do
    ...
    describe "with valid params" do
      it "updates the requested invitation" do
        Invitation.should_receive(:find).with("37") { mock_invitation }
        mock_invitation.should_receive(:update_attributes).with({'these' => 'params'})
        put :update, :id => "37", :invitation => {'these' => 'params'}
      end

      it "assigns the requested invitation as @invitation" do
        Invitation.stub(:find) { mock_invitation(:update_attributes => true) }
        put :update, :id => "1"
        assigns(:invitation).should be(mock_invitation)
      end
    ...
    describe "with invalid params" do
      it "assigns the invitation as @invitation" do
        Invitation.stub(:find) { mock_invitation(:update_attributes => false) }
        put :update, :id => "1"
        assigns(:invitation).should be(mock_invitation)
      end
